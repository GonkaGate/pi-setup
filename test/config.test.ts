...
test("creates the managed GonkaGate Pi provider", () => {
  ...
  assert.equal(
    provider.models.some((model) => model.id === RECOMMENDED_MODEL_ID),
    true,
  );
  assert.deepEqual(
...
test("curated model registry has exactly one recommended model", () => {
  assert.deepEqual(
    CURATED_MODELS.filter((model) => model.recommended).map(
      (model) => model.id,
    ),
    [RECOMMENDED_MODEL_ID],
  );
...
test("curated model registry records every Pi metadata decision", () => {
  for (const model of CURATED_MODELS) {
    assert.equal(typeof model.id, "string");
    assert.equal(typeof model.name, "string");
    assert.deepEqual(Object.keys(model.piFieldDecisions).sort(), [
      "compat",
      "contextWindow",
      "cost",
      "input",
    ...
    assert.deepEqual(
      Object.values(model.piFieldDecisions),
      Array(6).fill("use-pi-default-v1"),
    );
  }
});
